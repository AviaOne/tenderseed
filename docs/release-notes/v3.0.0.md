...
- **A `max_num_outbound_peers` of zero is now refused.** It used to start a
  seed that dialled nothing, therefore verified nothing, therefore had nothing
  to serve, while every line it logged looked normal. With the two refusals
  above, these are the only two behaviour changes on the Cosmos side.
...
namespace must be given a namespace, and one setting `max_num_outbound_peers`
to zero must be given a positive value, before this binary will start.
